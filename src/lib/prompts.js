...
REQUIREMENTS:
- All key screens arranged in ONE 16:9 presentation board. Use a 2x2 grid for
  4 screens; adjust the grid to the actual screen count.
- Each screen must show its REAL controls (upload zones, chips, selectable
  cards, buttons, checklists) exactly per the blueprint.
- Write the full design system into the prompt: exact hex colors, the type
  pairing, and the mood.
- No unfilled placeholders like [APP NAME] — everything concrete.
- Style: professional Dribbble/Behance portfolio presentation, crisp modern
  web UI, realistic proportions.
...
5. SCREEN-BY-SCREEN SPEC: every screen, every control, every button label in
   Thai, every state transition — exactly per the approved blueprint.
6. AI CALL SPEC: each Gemini call's model type (vision / text / image
   ...
   defensive parsing (strip code fences, try/catch, retry button).
7. Explicit final instruction: build the ENTIRE app polished and working — no
   placeholders, no TODOs, every button functional.
`.trim()
...
export function blueprintUser({ workflowPrompt, appName, theme, features }) {
  return [
...
export function buildUser({ workflowPrompt, blueprint }) {
  return [
